...
write_file <- function(
  ...
  # check for error
  if (length(software) > 1 || length(ext) > 1) stop("Error : please check that only one object was passed to software and ext argument")
  ...
  if (not_in(ext, c("csv", "xlsx", "xls", "tsv"))) stop("This file extension is not supported. Please select file among csv, xlsx, xls or tsv")
